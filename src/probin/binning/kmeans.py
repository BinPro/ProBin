...
def _clustering(cluster_count, max_iter, run, epsilon, verbose, log_probabilities_func, fit_nonzero_parameters_func, p, **kwargs):
    ...
    if not np.any(p):
        #p = _generate_centroids(cluster_count, len(kwargs["contigs"][0]),rs)
...
def _maximization(fit_nonzero_parameters_func, cluster_ind, centroids,rs,contigs,**kwargs):
    ...
    for i in xrange(len(centroids)):
        ...
        if not indexes.any():
            indexes[rs.randint(0,len(contigs))] = True
...
def _generate_kplusplus(log_probabilities_func,fit_nonzero_parameters_func,c_count,rs,contigs,**kwargs):
    ...
    contig_ind = np.zeros(contigs_ind.shape) == 1
    contig_ind[rs.randint(0,len(contigs_ind))] = True
    contigs_ind[contig_ind] = False
    centroids[0] = fit_nonzero_parameters_func(contigs=contigs[contig_ind],**kwargs)
    for centroids_index in xrange(1,c_count):
        ...
        contig_ind[:] = False
        contig_ind[furthest] = True
        centroids[centroids_index] = fit_nonzero_parameters_func(contigs=contigs[contig_ind],**kwargs)
    return centroids
