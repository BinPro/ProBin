...
def fit_nonzero_parameters(dna_l,kmer_hash_count):
    ...
    alpha_fit = fmin_l_bfgs_b(neg_log_density_l,alpha0,args=(dna_l,),maxfun=10,bounds=alpha_bounds, approx_grad=True)
    return alpha_fit[0]

def log_probability(seq,alpha):
    return log_density(seq,alpha)

def log_density(seq,alpha):
    A = sum(alpha)
...
def neg_log_density_l(alpha,dna_l):
    return -sum([log_density(dna,alpha) for dna in dna_l])
